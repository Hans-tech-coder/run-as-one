...
- **PromoCode** — a discount an organizer hands out, redeemed at checkout.
  ...
  (`buyQuantity`/`getQuantity`). There used to be four; `PERCENTAGE`, `FIXED`
...
label) · `/admin/marketing` (promotions: the kind, the event it is scoped to, what it
...
- **A promotion can reprice a race's own options** (Category Prices). The
  ...
  in all three places a price per option appears: the event page's category
  list, the "What's Included" headings above it, and the picker in the
  registration wizard, all through `components/CategoryPrice.tsx`. **In the
