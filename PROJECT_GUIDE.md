...
**Batch 1 of the pacer plan is in (2026-09-22)**: migration
...
`/admin/marketing` and from `spendByCode`. **A pacer code exists but checkout
does not accept one yet** — `discountAmountFor` returns 0 for `PACER` until
Batch 2 — so **`dev` must not be promoted to `main` until Batch 2 lands**.
Releasing it will also need `npx prisma migrate deploy` against production. Each file holds its decisions,
