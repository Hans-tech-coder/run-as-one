...
- **Organizer** — the client account. `role` is `ORGANIZER` or `SUPER_ADMIN`;
  ...
  reason a rejection was given, required for one and cleared by any later
  decision — and **`statusChangedAt`**, when a super admin last moved the status
...
rejection is saved. Every decision is announced with a toast. **A decided
...
**`SUPERADMIN_APPLICATIONS_PLAN.md` is an open queue, with Batches 1–2 landed.**
...
**Batch 3 (decision emails) is next, and needs the owner's answer first** on
whether the rejection reason is sent to the applicant; until then the dialog
says only that the reason is kept with the decision.
