...
- **Runner** — one participant on an order: `runnerNo` (their 1..n position on
  ...
  backfilled). Indexed on `categoryId`, which is how
...
**`GUARDIAN_CONSENT_PLAN.md` is in progress — Batches 1, 2 and 3 of 4 have
...
promoted to `main`**. Organizers cannot see any of it yet; that is Batch 4
(registrant detail, printable consent, CSV, runner edit, email).
