...
- **Organizer** — **Run As One itself, the one tenant** (`ADMIN_MERGE_PLAN.md`).
  ...
  reads it; `adminFee` is **a dead column** — every order's fee comes from
  `Event.adminFee` — and both were kept by the owner's call when Batch 5
  dropped the rest. **Batch 5 (migration `20260917180000_retire_super_admin`)
...
- **SiteSettings** — site-wide settings staff edit at `/admin/settings`, **one
  row keyed `"site"`**. Today only `contactEmail`, the admin email the whole app
  shows and mails from. No row means the defaults in `site-contact.ts`, so it
  works before anybody saves. Read only through `site-settings.ts` (§5).
  Migration `20260918090000_site_settings`, additive; a few dozen bytes.
...
| `site-contact.ts` | Site name, `SITE_URL`, legal "last updated", social channels, and the contact email's **default** (`DEFAULT_CONTACT_EMAIL`) and rules: `EMAIL_SENDING_DOMAIN` (the one domain Resend is verified for), `canSendFrom(email)`, `supportMailto(email)`. Client-safe. **Site-wide details belong here** or in `site-settings.ts` — never inline them in a component. |
| `site-settings.ts` | **The saved site-wide settings** (server-only; reads the `SiteSettings` row). `getContactEmail()` / `getSiteSettings()` — cached with `unstable_cache` under `SITE_SETTINGS_TAG`, which `PATCH /api/admin/site-settings` expires on save, and **never throws**: no row or a failed read falls back to `DEFAULT_CONTACT_EMAIL`. Server components call it; client components read `useContactEmail()` from `components/SiteContactProvider.tsx`, which the root layout fills. `email.ts` reads it per email for the footer, `replyTo` and (via `canSendFrom`) the sender, so its builders return a `Promise<EmailMessage>`. **Never show the address from a constant.** |
...
| `admin/site-settings` | PATCH | **The admin email**, `{ contactEmail }`. `platform:manage` (`platformActor()`). Trimmed and shape-checked (`errors.contactEmail`, 400); an unchanged value writes nothing. Upserts the `SiteSettings` row and writes `settings.contact_email.changed` (on the *Site settings* Activity shelf) in one transaction, then `revalidateTag(SITE_SETTINGS_TAG, { expire: 0 })` so the next page and email already carry it. Answers `{ settings: { contactEmail } }` |
...
- **A client viewer reaches registrant counts of its own client's events and
  ...
  `admin/profile`, `admin/profile/avatar` and `admin/profile/password` serve it. **A new dashboard page
...
7. **Site-wide contact and social details live in `lib/site-contact.ts`**, or in
   `lib/site-settings.ts` once staff can edit them (the admin email already is)
...
in `SETTINGS_PLAN.md`** (one batch per session): social links, sign out every
other device for the owner, and the owner-only default platform fee.
...
- The **admin email is a setting** (`/admin/settings`, `SiteSettings`); the
  social links are still constants awaiting the same, and the icons point at
  `/coming-soon`. **Releasing it needs `npx prisma migrate deploy` against
  production** for `20260918090000_site_settings`.
