...
As One settles with the client afterwards (tracking that is
`ADMIN_MERGE_PLAN.md` Batch 6). The per-runner admin fee is set on each event
(`Event.adminFee`).
...
| `RESEND_API_KEY` | Resend — sends every email in the app (registration emails, team and client invitations) from `info@cresendorunningcommunity.com`. Unset in dev just skips the send and reports it as not sent (see `lib/email.ts`). **Set in the local `.env` too, so an invitation sent on localhost reaches the real inbox** |
...
| `site-contact.ts` | Site name, contact email, legal "last updated", social channels. **Site-wide details belong here**, destined to become platform settings staff can edit — never inline them in a component. |
...
7. **Site-wide contact and social details live in `lib/site-contact.ts`**, headed
   for platform settings staff can edit — never inline in a component.
...
- Site contact and social links are constants awaiting a **platform settings
