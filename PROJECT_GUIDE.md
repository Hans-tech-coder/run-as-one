...
- **PromoCode** — a discount an organizer hands out, redeemed at checkout.
  ...
  **`PERCENTAGE`** or **`FIXED`** (the per-runner kinds, brought back
  ...
  amount is capped at that runner's entry). `discountValue` is 0 for the two
  older kinds. The per-runner kinds are claimed by one shared code or
...
- **PromoCategory** — one category a `PERCENTAGE` or `FIXED` promotion is
  restricted to: `promoCodeId`, `categoryId`, unique per pair, cascading on
