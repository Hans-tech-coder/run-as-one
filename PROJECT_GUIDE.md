...
- **Client** — an organization Run As One runs races for (`ADMIN_MERGE_PLAN.md`).
  ...
  **copied every applicant `Organizer` row into a `Client` with the same id** —
  ...
  the rest `NEW`. The Organizer originals stay until Batch 5. Tens of rows of
  short text; not a storage decision.
...
full-height `.admin-modal-sheet` below `sm`, and carries the decisions in its
footer. **The decisions come from `decisionsFrom`** (`organizer-status.ts`), so the
row, the card and the panel offer exactly what the route accepts: a pending
application is approved or **rejected**, an approved account suspended, a
suspended or rejected one approved. Approve and suspend ask the shared
`confirm`; **Reject opens `organizers/RejectDialog.tsx`** — `AlertModal` with a
labelled textarea, a counter and a `FieldError`, which refuses an empty or
too-short reason as Reject is pressed and lands the route's own refusal in the
same place. It stays open (`AlertModal`'s `busy`, with `BusyLabel`) until the
rejection is saved. **Its label says the applicant will read the reason**, because
the rejection email quotes it. **Approving and rejecting email the applicant**
(the route sends after saving): a sent email is announced in the toast (*"… The
applicant has been emailed at …"*); **one that did not go out is an error
alert** naming Resend's reason and the applicant's email and phone, because the
decision stands and they still have to hear about it. A rejection's report waits
until its dialog has closed. A suspension emails nobody and is announced plainly. **A decided
account opens on *The decision*** — the status, when it moved, and a
rejection's reason; a row decided before those columns existed shows only its
...
**`SUPERADMIN_APPLICATIONS_PLAN.md` holds the optional Batches 5–6; Batches 1–4 have landed.**
Batch 1 put the whole application in a panel on `/superadmin/organizers` and
removed the dead fee editor. **Batch 2 is in too:** the `REJECTED` status with a
required written reason, `lib/organizer-status.ts`, the sign-in check inverted to
an allowlist (a rejected applicant cannot sign in), the fourth chip, the reject
dialog and *The decision* in the panel. Migration
`20260916120000_organizer_status_decision` (`statusNote`, `statusChangedAt`) —
**run `npx prisma migrate deploy` against production when this is released.**
**Batch 3 is in too:** approving and rejecting email the applicant, the rejection
quoting its reason — the owner decided the reason is sent, one box — and the
screen reports an email that did not go out. No migration; the optional resend
action and `decisionEmailSentAt` were not built. **Batch 4 is in too:** every
decision writes one audit row (`organizer.approved` / `.rejected` / `.suspended` /
`.reinstated`) into the super admin's own trail — never the organizer's
`/admin/activity` — with the organizer as `entityId`, in the same transaction as
the status, and **`/superadmin/activity` reads them back** (the organizer
Activity screen in `scope="platform"`). No migration. **All four required batches have landed;**
Batches 5 and 6 are optional.
