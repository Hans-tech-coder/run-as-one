...
| **Run As One staff** (owner and admins, `platform:manage`) | Everything organizers do, plus read client submissions and **send the invite** that lets one sign in, curate the shared running-club list, read feedback and watch the platform fees collected — in the same dashboard. There is no separate super admin portal since `ADMIN_MERGE_PLAN.md` Batch 2; old `/superadmin/**` addresses redirect. Nothing is approved or rejected any more (Batch 3) | `/admin/**` |
| **Clients** (organizations Run As One runs races for) | Apply through `/admin/register` — no password — and wait on `/admin/clients`. Staff press **Send invite**; the contact chooses a password from the email and signs in as a **client viewer**, which sees its own events' registrant counts and nothing else (that screen is Batch 4) | `/admin/register`, `/admin/invite/[token]`, `/admin` |
...
| `RESEND_API_KEY` | Resend — sends every email in the app (registration emails, team invitations, organizer decisions) from `info@cresendorunningcommunity.com`. Unset in dev just skips the send and reports it as not sent (see `lib/email.ts`). **Set in the local `.env` too, so deciding an application on localhost emails the real applicant** |
...
- **Organizer** — the client account. `role` is `ORGANIZER` or `SUPER_ADMIN`;
  `status` is `PENDING` / `APPROVED` / `REJECTED` / `SUSPENDED`
  (`lib/organizer-status.ts`, still plain text), with **`statusNote`** — the
  reason a rejection was given, required for one, **quoted to the applicant in
  the rejection email**, and cleared by any later decision — and **`statusChangedAt`**, when a super admin last moved the status
  (null on rows decided before it existed; `updatedAt` cannot stand in, a
  profile edit moves it). `adminFee` (centavos) is **a dead column**:
  ...
  **Every one of them is nullable**, because the accounts that pre-date the
  ...
  a row that says nothing, and `hasApplicationDetails()` is what asks. **No
  application writes these any more** — since `ADMIN_MERGE_PLAN.md` Batch 3 an
  application is a `Client` row (below), and the Organizer copies are dropped
  in Batch 5. The
...
- **Event** — title, unique **`slug`**, `date` (**string `YYYY-MM-DD`**, not
  ...
  *that* pickup existed), certificate template + coordinates, and **`clientId`** — which client the race
  is run for (null = not linked yet, which every pre-existing event is until
  staff link it in `ADMIN_MERGE_PLAN.md` Batch 5; a null is invisible to every
  client viewer). Owns `Category[]`, `BankAccount[]`, `Registration[]`,
  `RaceResult[]`.
...
- **Client** — an organization Run As One runs races for (`ADMIN_MERGE_PLAN.md`).
  ...
  `ARCHIVED`, plain text — `lib/client.ts`), `invitedAt`, and the same
  application columns `Organizer` carries (`orgType` … `applicationNote`, all
  nullable). Owns `Event[]` (through `Event.clientId`) and `viewers`
  ...
  **copied every applicant `Organizer` row into a `Client` with the same id** —
  ...
  membership, so Run As One's row and the super admin are left out by what they
  hold rather than by id; rejected and suspended applicants became `ARCHIVED`,
  the rest `NEW`. The Organizer originals stay until Batch 5. **Since Batch 3
...
| `organizer-status.ts` | **Where an organizer account stands, and what each standing lets it do.** `ORGANIZER_STATUSES` (`PENDING`, `APPROVED`, `REJECTED`, `SUSPENDED`) with their labels and badge tones (`ORGANIZER_STATUS_COPY`, `organizerStatusLabel` / `organizerStatusBadge`, falling back to the stored text), `asOrganizerStatus` guarding the PATCH door, and **`organizerCanSignIn` — an allowlist: only `APPROVED` signs in.** It replaced a blocklist of PENDING and SUSPENDED in `actor.ts` and `auth/login`, which would have let a `REJECTED` applicant straight in; now a status added later is refused until somebody decides otherwise. `auth/login`, `getActor()`, `activeMembershipWhere` and both invitation surfaces all read it. **`decisionsFrom(status)`** is which decisions a status allows, and the screen's buttons and the route's `canDecide` both read it: Pending → approve or reject, Approved → suspend, Suspended or Rejected → approve; nothing moves an account back to Pending, a rejection is never offered on an account that was opened, and a suspension never on one that was not. `readStatusNote` is the reason rule (required, 10–1,000 characters) the reject dialog and the route share. |
...
`ADMIN_MERGE_PLAN.md` Batch 2**, and Clients replaced its organizer accounts
screen in Batch 3. They sit in the one dashboard's sidebar for owners and
...
controls and `RejectDialog` were deleted with the organizers screen; the
`admin/organizers` routes stay until Batch 5 and nothing calls them) · `/admin/communities` (approve, rename, reject clubs; the
...
- **A client viewer reaches registrant counts of its own client's events and
  nothing else**, enforced in `actor.ts` rather than by hidden links: every
  existing page and route checks `event:view` or a stronger permission, which a
  viewer does not hold, so it sees empty lists and "not found" everywhere today.
  A surface built for viewers (Batch 4) must ask `event:view-summary` and pass
...
**`ADMIN_MERGE_PLAN.md` is the active queue — Batches 1–3 have landed (2026-09-17).**
...
organizer approval routes, columns and emails still exist in code and are
retired in Batch 5 (Batch 3 removed the approval screen). **Batch 1:** the production
...
localhost with throwaway `local-dev` rows (plan's Batch 3 notes). **Batch 4
(the Viewer dashboard) is next** — until it lands a viewer who signs in sees
the staff Overview's empty shell and 404s.
...
panel lives on there; the rest is retired in Batch 5.
