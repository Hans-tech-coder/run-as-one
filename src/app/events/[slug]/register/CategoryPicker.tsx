 ...
 *
 * One list for both shapes. A race sells distances and a fun run sells
 * inclusion packages, but to the runner it is the same decision — pick the one
 * row you are paying for — so it gets the same control. The races used to
 * render as a card grid, which read as a different widget on the same step and
 * left a half-empty last row whenever the distance count was not a multiple of
 * three. Stacked full-width rows also give every price one right edge to align
 * to, which is what makes the options comparable at a glance.
 *
 * The only thing a race carries that a package does not is the distance, shown
 * as a chip beside the name; a package simply has no chip.
 *
  ...
  return (
    <>
      {/* tabIndex -1 so the summary dialog can send the caret to the group as a
          whole. The radios are many; the heading is the one place that means
          "start here". */}
      <div
        id={id}
        tabIndex={-1}
            ...
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                ...
                {/* after:inset-0 stretches the label's hit area over the whole
                    ...
                    the option too. Without it only the text row was clickable
...
function PosterThumb({
  ...
  return (
    <button
      type="button"
      onClick={e => {
        // The row's label overlay also answers clicks in this area, so the
        // poster has to keep its own from selecting the option.
        e.stopPropagation();
        onOpen();
      }}
      aria-label={`View ${cat.name} inclusions`}
      className={`w-16 h-16 sm:w-24 sm:h-24 relative shrink-0 rounded-xl overflow-hidden border border-white/10 bg-black/40 group/thumb ${className}`}
    >
      <img
        src={cat.imageUrl}
        alt=""
